...
class TopPerformersDialog extends StatefulWidget {
  ...
  final List<TopPerformerModel> performers;
  ...
  static Future<void> show(BuildContext context, {required String userId, required VoidCallback onViewTeam}) async {
    ...
    // Fetch performers before showing dialog
    try {
      final apiClient = GetIt.instance<ApiClient>();
      final response = await apiClient.getTopPerformers(userId);
      
      if (response['status'] == true && response['data'] != null) {
        final List data = response['data'];
        final List<TopPerformerModel> performers = data
            .take(3)
            .map((e) => TopPerformerModel.fromJson(e))
            .toList();

        if (performers.isEmpty) {
          // Don't show anything if no performers
          AppGlobals.hasShownTopPerformersBanner = true;
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...widget.performers.asMap().entries.map((entry) {
                            ...
                            children: [
                              Container(
                                ...
                                child: CircleAvatar(
                                  ...
                                  backgroundImage:
                                      performer.profilePhoto.isNotEmpty
                                          ? NetworkImage(
                                              "https://workiees.com/${performer.profilePhoto}")
                                          : null,
