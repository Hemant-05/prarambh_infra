...
class _AdvisorDashboardScreenState extends State<AdvisorDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: false,
        title: Text('Business Dashboard', style: GoogleFonts.montserrat(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white)),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildPendingActionsHeader(provider.data!.pendingActions.length),
                    _buildPendingActionsList(provider.data!.pendingActions),
                    const SizedBox(height: 24),

                    _buildSectionTitle('Promotion Status'),
                    _buildPromotionStatusTable(provider.data!.promotionStatus),
                    const SizedBox(height: 24),

                    _buildActiveContestsHeader(),
                    _buildActiveContestsList(provider.data!.activeContests),
          ...
          child: Row(
            children: [
              Stack(
                clipBehavior: Clip.none,
                children: [
                  const CircleAvatar(radius: 35, backgroundImage: NetworkImage('https://i.pravatar.cc/150?img=11')), // Replace with actual image
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text('ACTIVE', style: GoogleFonts.montserrat(fontSize: 9, color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                  ),
                ],
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [Icon(Icons.badge_outlined, size: 14, color: Colors.grey[600]), const SizedBox(width: 4), Text('ID: ${data.advisorId}', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey[700]))]),
                    const SizedBox(height: 4),
                    Row(children: [Icon(Icons.person_outline, size: 14, color: Colors.grey[600]), const SizedBox(width: 4), Text('Parent: ', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey[700])), Text(data.parentName, style: GoogleFonts.montserrat(fontSize: 12, color: Colors.blue[700], fontWeight: FontWeight.w600))]),
                  ],
                ),
              )
            ],
  ...
  Widget _buildCareerProgress(AdvisorDashboardModel data) {
    return Container(
      padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: _primaryBlue.withOpacity(0.3))),
      child: Row(
        children: [
          Container(
            width: 60, height: 60, decoration: BoxDecoration(color: const Color(0xFF1E2A47), borderRadius: BorderRadius.circular(12)),
            child: Stack(
              alignment: Alignment.center,
              children: [
                const Icon(Icons.star_border, color: Colors.white, size: 30),
                Positioned(bottom: 4, child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(10)), child: Text('LVL 3', style: GoogleFonts.montserrat(fontSize: 9, fontWeight: FontWeight.bold)))),
              ],
            ),
          ),
          const SizedBox(width: 16),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Row(children: [Text('NEXT: ${data.nextLevel}', style: GoogleFonts.montserrat(fontSize: 10, fontWeight: FontWeight.bold, color: _primaryBlue)), const SizedBox(width: 4), Icon(Icons.arrow_outward, size: 12, color: _primaryBlue)]),
              ],
            ),
          ),
  ...
  Widget _buildSalesConversionHeader() {
    return Row(
      ...
      children: [
        ...
        Container(
          padding: const EdgeInsets.all(2), decoration: BoxDecoration(color: Colors.grey.shade200, borderRadius: BorderRadius.circular(20)),
          child: Row(
            children: ['Month', 'Quarter', 'Year'].map((e) => GestureDetector(
              onTap: () => setState(() => _selectedSalesTab = e),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                decoration: BoxDecoration(color: _selectedSalesTab == e ? Colors.white : Colors.transparent, borderRadius: BorderRadius.circular(20), boxShadow: _selectedSalesTab == e ? [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 4)] : []),
  ...
  Widget _buildSalesCard(String value, String label, Color bgColor, Color textColor) {
    return Container(
      padding: const EdgeInsets.symmetric(vertical: 16), decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(12)),
      child: Column(
        children: [
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: iconColors[index % iconColors.length].withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Icon(icons[index % icons.length], color: iconColors[index % iconColors.length])),
  ...
  Widget _buildPromotionStatusTable(List<PromotionMetric> metrics) {
    return Container(
      ...
      child: Column(
        children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: const BorderRadius.vertical(top: Radius.circular(12))),
            child: Row(
              children: [
          ...
          ...metrics.map((m) => Container(
            ...
            child: Row(
              children: [
                Expanded(flex: 2, child: Row(children: [Text(m.metric, style: GoogleFonts.montserrat(fontSize: 13, fontWeight: m.metric.contains('Team Booking') ? FontWeight.bold : FontWeight.w500)), if (m.metric.contains('Team Booking')) const Padding(padding: EdgeInsets.only(left: 6), child: Icon(Icons.circle, size: 8, color: Colors.blue))])),
  ...
  Widget _buildActiveContestsHeader() {
    return Row(
      ...
      children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(4)), child: Text('RUNNING', style: GoogleFonts.montserrat(fontSize: 10, fontWeight: FontWeight.bold, color: _primaryBlue))),
      ],
    );
  }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return Container(
          ...
          child: ListTile(
            leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(8)), child: Icon(icons[index % icons.length], color: _primaryBlue)),
  ...
  Widget _buildSectionTitle(String title) => Text(title, style: GoogleFonts.montserrat(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87));
  ...
  Widget _buildDrawer() {
    return Drawer(
      child: ListView(
        ...
        children: [
          Container(
            padding: const EdgeInsets.only(top: 60, bottom: 20), color: Colors.white,
            child: Column(
              children: [
                const CircleAvatar(radius: 40, backgroundImage: NetworkImage('https://i.pravatar.cc/150?img=11')),
                const SizedBox(height: 12),
                Text('Rajesh Kumar', style: GoogleFonts.montserrat(fontSize: 18, fontWeight: FontWeight.bold)),
                Text('MANAGER : #PI-8821', style: GoogleFonts.montserrat(fontSize: 12, color: _primaryBlue, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
          const Divider(),
          _drawerItem(Icons.description_outlined, 'Document View',(){
            Navigator.push(context, MaterialPageRoute(builder: (context) => DocumentCenterScreen(),));
          }),
          ...
          _drawerItem(Icons.calculate_outlined, 'Calculator - INSTALLMENT',(){}),
