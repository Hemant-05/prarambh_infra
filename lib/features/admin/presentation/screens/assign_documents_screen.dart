...
class _AssignDocumentsScreenState extends State<AssignDocumentsScreen> {
  ...
  Future<void> _pickFile(String type) async {
    ...
    if (result != null) {
      setState(() {
        if (type == 'Welcome Letter') welcomeLetter = File(result.files.single.path!);
        if (type == 'Application Form') applicationForm = File(result.files.single.path!);
  ...
  Future<void> _submitDocuments() async {
    ...
    if (mounted) {
      if (successCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$successCount documents assigned successfully!')));
        Navigator.popUntil(context, ModalRoute.withName('/admin_dashboard'));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select at least one document to assign.')));
      }
    }
  }
        ...
        child: SafeArea(
          child: ElevatedButton.icon(
            ...
            label: isSaving
                ...
                : Text('Confirm and Send', style: GoogleFonts.montserrat(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Attach Custom Documents', style: GoogleFonts.montserrat(fontWeight: FontWeight.bold, fontSize: 16, color: textColor)),
                  const SizedBox(height: 16),
                  _buildDocRow('Welcome Letter', welcomeLetter, primaryBlue, textColor),
                  const Divider(),
                  _buildDocRow('Application Form', applicationForm, primaryBlue, textColor),
                  const Divider(),
  ...
  Widget _buildDocRow(String title, File? selectedFile, Color primaryBlue, Color textColor) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8),
      child: Row(
        children: [
          Container(
            padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.blue[50], borderRadius: BorderRadius.circular(8)),
            child: Icon(selectedFile != null ? Icons.check_circle : Icons.description, color: primaryBlue),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                if (selectedFile != null)
                  Text(selectedFile.path.split('/').last, style: GoogleFonts.montserrat(fontSize: 10, color: Colors.green), maxLines: 1, overflow: TextOverflow.ellipsis)
                else
                  Text('Not Selected', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey[500])),
              ],
            ),
          ),
          ElevatedButton.icon(
            onPressed: () => _pickFile(title),
            icon: Icon(selectedFile != null ? Icons.edit : Icons.upload, size: 14, color: Colors.white),
            label: Text(selectedFile != null ? 'Change' : 'Select', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.white)),
            style: ElevatedButton.styleFrom(backgroundColor: selectedFile != null ? Colors.green : primaryBlue, padding: const EdgeInsets.symmetric(horizontal: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6))),
          ),
        ],
      ),
    );
