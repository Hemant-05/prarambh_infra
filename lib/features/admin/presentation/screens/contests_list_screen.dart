...
class ContestsListScreen extends StatefulWidget {
  const ContestsListScreen({Key? key}) : super(key: key);
...
class _ContestsListScreenState extends State<ContestsListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Running Contests', style: GoogleFonts.montserrat(color: isDark ? Colors.white : Colors.black, fontWeight: FontWeight.bold, fontSize: 18)),
        actions: [IconButton(icon: Icon(Icons.filter_list, color: isDark ? Colors.white : Colors.black), onPressed: () {})],
      ),
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: ElevatedButton(
            ...
            child: Text('Create Contest', style: GoogleFonts.montserrat(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
          ),
        ),
      ),
      body: Column(
                ...
                return Container(
                  ...
                  child: Row(
                    children: [
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Text('${contest.status} • ${contest.rewardText}', style: GoogleFonts.montserrat(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey[600])),
                              ],
                            ),
                            const SizedBox(height: 8),
                            Text(contest.title, style: GoogleFonts.montserrat(fontSize: 18, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            _buildIconText(Icons.track_changes, contest.targetText),
                      ...
                      Container(
                        width: 90, height: 110,
                        decoration: BoxDecoration(color: Colors.blue[50], borderRadius: BorderRadius.circular(12), image: const DecorationImage(image: AssetImage('assets/images/logos.png'), fit: BoxFit.cover)),
                      )
                    ],
                  ),
                );
  ...
  Widget _buildFilterChip(String label, bool isActive, Color primaryBlue) {
    return Container(
      ...
      child: Text(label, style: GoogleFonts.montserrat(fontSize: 12, fontWeight: FontWeight.w600, color: isActive ? Colors.white : Colors.grey[700])),
    );
  }
  ...
  Widget _buildIconText(IconData icon, String text) {
    return Row(
      children: [
        ...
        const SizedBox(width: 6),
        Text(text, style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey[600])),
      ],
    );
  }
}