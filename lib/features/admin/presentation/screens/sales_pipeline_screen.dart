...
class _SalesPipelineScreenState extends State<SalesPipelineScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _tabController.addListener(() {
      if (!_tabController.indexIsChanging) {
        ...
        context.read<AdminLeadProvider>().fetchPipelineLeads(stages[_tabController.index]);
      }
    });
      ...
      appBar: PreferredSize(
        ...
        child: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: const [Tab(text: 'Suspecting'), Tab(text: 'Prospecting'), Tab(text: 'Site Visit'), Tab(text: 'Booking')],
          ),
        ),
      ),
  ...
  Widget _buildPipelineCard(PipelineLeadModel lead, Color cardColor, Color primaryBlue, bool isDark) {
      ...
      child: Column(
        children: [
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Expanded(child: Text(lead.name, style: GoogleFonts.montserrat(fontWeight: FontWeight.bold, fontSize: 16, color: textColor))),
                        Expanded(child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4), decoration: BoxDecoration(color: Colors.blue[50], borderRadius: BorderRadius.circular(4)), child: Text('ADVISOR: ${lead.advisorName}', style: GoogleFonts.montserrat(fontSize: 8, fontWeight: FontWeight.bold, color: primaryBlue), overflow: TextOverflow.ellipsis,))),
                      ],
                    ),
                    const SizedBox(height: 4),
                    Text(lead.project, style: GoogleFonts.montserrat(fontSize: 13, color: primaryBlue, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Row(children: [Icon(Icons.calendar_today_outlined, size: 14, color: Colors.grey[500]), const SizedBox(width: 6), Text('Last: ${lead.lastActiveDate}', style: GoogleFonts.montserrat(fontSize: 12, color: Colors.grey[600]))]),
              GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LeadDetailsScreen(leadId: lead.id, leadName: lead.name))),
