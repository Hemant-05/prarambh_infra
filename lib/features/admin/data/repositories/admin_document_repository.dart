...
class AdminDocumentRepository {
  ...
  Future<List<ProjectDocumentModel>> getDocuments() async {
    try {
      final response = await apiClient.getProjectDocuments();
  ...
  Future<bool> uploadDocument(File file, String name, String category) async {
    try {
      final response = await apiClient.uploadProjectDocument(file, name, category);
